...
// VERSION:
// https://github.com/mozilla/gecko-dev/blob/6e6387eecb11d47e12d6d95d242799985a715f45/dom/push/PushCrypto.jsm
...
this.PushCrypto = {
  ...
  _deriveKeyAndNonce(ikm, salt, receiverKey, senderKey, authenticationSecret) {
    ...
    return kdfPromise.then(kdf => Promise.all([
      kdf.extract(concatArray([ENCRYPT_INFO, context]), 16).then(bits => {
       PushCrypto._KEY = bits; // XXX - exposed for debugging purposes (cek)
       return bits;
